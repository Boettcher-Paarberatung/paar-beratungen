...
class OsAgentHelper {
  ...
  public static function is_agent_available_on($agent_id, $start_date, $start_minutes, $duration, $service_id, $location_id){
    ...
    if(OsBookingHelper::is_timeframe_in_periods($start_minutes, $end_minutes, $booked_periods_arr)){
      $is_available = false;
    }
    if(!OsBookingHelper::is_timeframe_in_periods($start_minutes, $end_minutes, $work_periods_arr, true)){
      $is_available = false;
    }
    return $is_available;
  }
