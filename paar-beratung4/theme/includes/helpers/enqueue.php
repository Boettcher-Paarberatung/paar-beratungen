...
class Bunch_Enqueue
{
	...
	function bunch_enqueue_scripts()
	{
		...
		$dark_color = ( mindron_set( $options, 'website_theme' ) == 'dark' ) ? true : false;
		
		$dark_color = ( mindron_set( $_GET, 'color_style' ) == 'dark' ) ? true : $dark_color;
		
		if(mindron_set($options, 'color_scheme') == 'color2') : $color = 'css/color2.css'; else: $color = 'css/color1.css'; endif;	
		...
		if( is_single() ) {
			...
			if( $format == 'gallery' ) wp_enqueue_script( array( 'mindron_jquery-flexslider' ) );
			if( $format == 'video' || $format == 'audio' ) wp_enqueue_script( array( 'mindron_jquery-fitvids' ) );
		}
		if( is_singular( 'bunch_projects' ) || $wp_query->is_posts_page || is_search() || is_tag() || is_category() || is_author() || is_archive() ) 
  		wp_enqueue_script( array('mindron_jquery-flexslider', 'mindron_owl.carousel', 'mindron_jquery-fitvids') );
		wp_enqueue_script( array('mindron_custom_script') );
		
		wp_add_inline_style( 'mindron-main-style', $this->wp_head() );
		$opt = _WSH()->option();
		$h_js = 'if( ajaxurl === undefined ) var ajaxurl = "'.esc_url( admin_url( 'admin-ajax.php' ) ).'";';
		wp_add_inline_script( 'mindron-main-script', $h_js );
		
		
	}
	
	function mindron_get_font_settings( $styles )
	{
		$opt = _WSH()->option();
		$protocol = ( is_ssl() ) ? 'https' : 'http';
		$font = array();
		...
		if( mindron_set( $opt, 'use_custom_font' ) ){
			...
			if( $h1 = mindron_set( $opt, 'h1_font_family' ) ) $font[$h1] = urlencode( $h1 ).':400,300,600,700,800';
			if( $h2 = mindron_set( $opt, 'h2_font_family' ) ) $font[$h2] = urlencode( $h2 ).':400,300,600,700,800';
			if( $h3 = mindron_set( $opt, 'h3_font_family' ) ) $font[$h3] = urlencode( $h3 ).':400,300,600,700,800';
		}
		
		if( mindron_set( $opt, 'body_custom_font' ) ){
			if( $body = mindron_set( $opt, 'body_font_family' ) ) $font[$body] = urlencode( $body ).':400,300,600,700,800';
		}
	...
	function wp_head()
	{
		...
		$style .= mindron_set( $opt, 'header_css' );
