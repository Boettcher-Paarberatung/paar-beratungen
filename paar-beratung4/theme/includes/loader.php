...
add_action( 'init', 'mindron_bunch_theme_init');


if( !function_exists( 'mindron_set' ) ) {
	function mindron_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('mindron_printr' ) ) {
	function mindron_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('mindron__font_awesome' ) ) {
	function mindron_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = mindron_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('mindron_load_class' ) ) {
	function mindron_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
mindron_load_class( 'enqueue', 'helpers', false );
mindron_load_class( 'bootstrap_walker', 'helpers', false );


if( mindron_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function mindron_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include MINDRON_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include MINDRON_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = MINDRON_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = MINDRON_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( mindron_set( $_GET, 'page' ) == 'mindron'.'_option' ) 
		{
			if( mindron_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'mindron_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = MINDRON_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'mindron'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'mindron'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'mindron' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'mindron' ), // menu label
	));
