...
psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-general.php', true);
psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-post.php', true);
psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-page.php', true);
psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-other.php', true);


if (class_exists('Cmsmasters_Projects')) {
	psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-project.php', true);
}


if (class_exists('Cmsmasters_Profiles')) {
	psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-profile.php', true);
}


if (CMSMASTERS_DONATIONS) {
	psychology_help_locate_template('cmsmasters-donations/admin/cmsmasters-theme-options-campaign.php', true);
	psychology_help_locate_template('cmsmasters-donations/admin/cmsmasters-theme-options-donation.php', true);
}


psychology_help_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-product.php', true);


if (!function_exists('psychology_help_get_custom_all_meta_fields')) {
function psychology_help_get_custom_all_meta_fields() {
	...
	return apply_filters('psychology_help_get_custom_all_meta_fields_filter', $custom_all_meta_fields);
}
}


function psychology_help_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_localize_script('cmsmasters_theme_options_js', 'cmsmasters_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'psychology-help'), 
			'select_format' => 		esc_attr__('Please select the format.', 'psychology-help'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'psychology-help'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'psychology-help'), 
...
add_action('admin_enqueue_scripts', 'psychology_help_admin_enqueue_scripts');


function show_cmsmasters_meta_box() {
	global $post;
	
	$custom_all_meta_fields = psychology_help_get_custom_all_meta_fields();
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'psychology-help') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'psychology-help') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'psychology-help') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'psychology-help') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'psychology-help') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'psychology-help') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'psychology-help') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'psychology-help') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'psychology-help') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'psychology-help') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'psychology-help') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'psychology-help') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'psychology-help') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'psychology-help') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'psychology-help') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'psychology-help') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'psychology-help') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'psychology-help') : esc_attr__('Create Gallery', 'psychology-help')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'psychology-help') . '" data-button="' . esc_attr__('Insert Gallery', 'psychology-help') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'psychology-help') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'psychology-help') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'psychology-help') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function add_custom_cmsmasters_meta_box() {
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'psychology-help'), 
