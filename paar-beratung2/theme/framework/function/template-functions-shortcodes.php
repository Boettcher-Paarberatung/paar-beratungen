...
function psychology_help_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
	$out = '';
...
function psychology_help_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function psychology_help_slider_post_check_exc_cont($type = 'post') {
	$exc = psychology_help_slider_post_exc_cont($type, false);
...
function psychology_help_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function psychology_help_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
...
function psychology_help_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function psychology_help_slider_post_like($type = 'post', $show = true) {
	$out = '';
...
function psychology_help_get_slider_post_views($type = 'post', $show = true) {
	if ($type == 'post') {
...
function psychology_help_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = psychology_help_get_comments('cmsmasters_slider_post_comments');
		} elseif ($type == 'project') {
			$out = psychology_help_get_comments('cmsmasters_slider_project_comments');
		}
	}
...
function psychology_help_slider_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_attr__('Read More', 'psychology-help');
	}
	
	
	$out = '<a class="cmsmasters_slider_post_read_more button" href="' . esc_url(get_permalink($cmsmasters_id)) . '">' . esc_html($cmsmasters_post_read_more) . '</a>';
