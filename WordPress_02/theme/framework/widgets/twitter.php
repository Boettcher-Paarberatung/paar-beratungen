...
require_once 'TwitterAPP/TwitterOAuthException.php';
...
class Jws_Theme_Tweets_Widget extends WP_Widget {
    ...
    function Jws_Theme_Tweets_Widget()
    {
        ...
        parent::__construct('ro-tweets-widget', __('Twitter', 'medicare'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
		...
		wp_enqueue_script( 'owl.carousel.min', JWS_THEME_URI_PATH.'/assets/vendors/owl-carousel/owl.carousel.min.js', array('jquery') );
		
        echo balanceTags($before_widget);

        if($title) {
            echo balanceTags($before_title.'<i class="fa fa-twitter"></i>'.$title.$after_title);
        }
        ...
        if($consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) {
            ...
            if($twitter && is_array($twitter)) {
                ...
                ?>
				<div class="jws-theme-twitter-widget jws-theme-twitter-widget1">
						<ul>
							<?php foreach($twitter as $tweet):
								?><li class="jws-theme-tweet"> <?php  
									$tweet = (array) $tweet;
									if( ! empty( $tweet ) ):
								...
								?>
									<div class="jws-theme-content">
										<div class="jws-theme-tweet-text">
											<?php 
												echo '<span class="jws-theme-name">@'.$tweet['user']['name'].': </span>';
											?>
											<?php echo $the_tweet ?>
										</div>
