						...
						<div id="tb-list-porfolio" class="<?php echo esc_attr(implode(' ', $class)); ?>">
								...
								<?php if( $show_filter ) { ?>
									...
									<ul class="controls-filter list-unstyled list-inline text-center">
										...
										<li class="filter active" data-filter="all"><a href="javascript:void(0);"><?php _e('All Work', 'medicare');?></a></li>
									...
									<?php if(  $show_viewmore && $wp_query->max_num_pages > 1 && $paged < $wp_query->max_num_pages  ) {
									 ...
									 ?>
										<div class="tb-viewmore text-center <?php if($show_pagination) echo 'has_pagination'; ?>">
											...
											<div class="tb-btn-viewmore"><a id="tb-btn-viewmore" class="btn-transparent" data-options='<?php echo esc_attr( json_encode( $atts ) );?>' data-args='<?php echo esc_attr( json_encode( $args ) );?>' href="#page/<?php echo intval( $paged + 1 );?>"><?php _e('View More','medicare');?></a></div>
