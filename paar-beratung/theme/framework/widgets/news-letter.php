 ...
 */
class Medicare_NewsletterWidget extends WP_Widget {

    function __construct() {
        parent::__construct(false, $name = '@Medicare Newsletter', array('description' => 'Newsletter widget to add subscription forms on sidebars'), array('width' => '350px'));
    }

    static function get_widget_form( array $instance=array() ) {
        $options_profile = get_option('newsletter_profile');
        $placeholder =( isset( $instance['placeholder'] ) ) ? $instance['placeholder'] : esc_html__('Email','medicare');
        ...
        $form .= '<p class="subscribe_email_input"><input class="newsletter-email" type="email" placeholder="'. esc_attr( $placeholder ) .'" required name="ne" value="' . esc_attr( $email ) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></p>';
        ...
        if (strpos($options_profile['subscribe'], 'http://') !== false) {
            $form .= '<p class="subscribe_email"><input class="newsletter-submit" type="image" src="' . $options_profile['subscribe'] . '"/></p>';
        } else {
            $form .= '<p class="subscribe_submit_button"><input class="newsletter-submit" type="submit" value="' . $options_profile['subscribe'] . '"/></p>';
        }
        // hide popup
        $form .= '<div class="tb-newsletter-checkbox"><input type="checkbox" name="hide_popup" id="tb-hide-popup"><span>' . esc_html__("Don’t show this popup again","medicare") .'</span></div>';
    ...
    function widget($args, $instance) {
        ...
        if (stripos($instance['text'], '<form') === false) {
            ...
            $form .= Medicare_NewsletterWidget::get_widget_form( $instance );
            $form .= '</div>';
            ...
            $buffer = str_ireplace('</form>', '<input type="hidden" name="nr" value="widget"/><div class="tb-newsletter-checkbox"><input type="checkbox" name="hide_popup" id="tb-hide-popup"><span>' . esc_html__("Don’t show this popup again","medicare") .'</span></div></form>', $buffer);
        }
    ...
    function form($instance) {
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id('image_url'); ?>">
                <?php esc_html_e('Image on the left side','medicare');?>:
            ...
            <label for="<?php echo $this->get_field_id('placeholder'); ?>">
                <?php esc_html_e('Placeholder','medicare');?>:
            ...
            <label for="<?php echo $this->get_field_id('text'); ?>">
                <?php esc_html_e('Introduction','medicare');?>:
        ...
        <p> <?php printf( esc_html__("Use the tag %s to place the subscription form within your personal text.","medicare"),"{subscription_form}");?>
        </p>
        <?php
    }
...
add_action('widgets_init', create_function('', 'return register_widget("Medicare_NewsletterWidget");'));
?>
