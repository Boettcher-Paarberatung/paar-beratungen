...
if ( ! function_exists( 'installSample' ) ) {
	function installSample(){
		...
		$themename = 'medicare';
		/* Import contents */
		$medicare = $wp_import->import( JWS_THEME_ABS_PATH_ADMIN. '/sample/'.$themename.'/sample.xml');
		// setup menus		
...
if(!function_exists('tb_import_revslider')){
		function tb_import_revslider($themename){
			if(class_exists('UniteBaseAdminClassRev')){
				require_once(ABSPATH .'wp-content/plugins/revslider/admin/revslider-admin.class.php');
				if ($handle = opendir(JWS_THEME_ABS_PATH_ADMIN.'/sample/'.$themename.'/revslider')) {
					while (false !== ($entry = readdir($handle))) {
						if ($entry != "." && $entry != "..") {
							$_FILES['import_file']['tmp_name'] = JWS_THEME_ABS_PATH_ADMIN.'/sample/'.$themename.'/revslider/'.$entry;
