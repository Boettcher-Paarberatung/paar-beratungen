...
class OsBookingModel extends OsModel{
  ...
  public function calculate_end_date(){
    if(($this->start_time + $this->get_duration()) > (24 * 60)){
  ...
  public function calculate_end_time(){
    $end_time = $this->start_time + $this->get_duration();
  ...
  public function get_start_timestamp($timeshift_minutes = false){
    return $this->format_start_date_and_time('U', $timeshift_minutes);
  }

  public function get_end_timestamp($timeshift_minutes = false){
    return $this->format_end_date_and_time('U', $timeshift_minutes);
  }
  ...
  public function get_upcoming_bookings($agent_id = false, $customer_id = false, $service_id = false, $location_id = false, $limit = 3){
    ...
    return $bookings->should_be_approved()->where($args)->set_limit($limit)->order_by('start_date asc, start_time asc')->get_results_as_models();
  ...
  public function format_end_date_and_time($format = "Y-m-d H:i:s", $shift_in_minutes = false){
    $end_date = OsTimeHelper::shift_date_by_minutes($this->end_date, $this->end_time, $shift_in_minutes);
    $end_time = OsTimeHelper::shift_time_by_minutes($this->end_time ,$shift_in_minutes);
    $date = OsWpDateTime::os_createFromFormat("Y-m-d H:i:s", $end_date.' '.OsTimeHelper::minutes_to_army_hours_and_minutes($end_time).':00');
    return $date->format($format);
  }

  public function format_start_date_and_time($format = "Y-m-d H:i:s", $shift_in_minutes = false){
    $start_date = OsTimeHelper::shift_date_by_minutes($this->start_date, $this->end_time, $shift_in_minutes);
    $start_time = OsTimeHelper::shift_time_by_minutes($this->start_time ,$shift_in_minutes);
    $date = OsWpDateTime::os_createFromFormat("Y-m-d H:i:s", $start_date.' '.OsTimeHelper::minutes_to_army_hours_and_minutes($start_time).':00');
    return $date->format($format);
  }
