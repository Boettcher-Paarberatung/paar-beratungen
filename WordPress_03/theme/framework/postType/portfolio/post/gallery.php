...
if (
	$cmsmasters_option['psychology-help' . '_portfolio_project_like'] || 
	$cmsmasters_option['psychology-help' . '_portfolio_project_date'] || 
	$cmsmasters_option['psychology-help' . '_portfolio_project_cat'] || 
	$cmsmasters_option['psychology-help' . '_portfolio_project_comment'] || 
	$cmsmasters_option['psychology-help' . '_portfolio_project_author'] || 
	$cmsmasters_option['psychology-help' . '_portfolio_project_tag'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project cmsmasters_' . $cmsmasters_project_columns); ?>>
	...
	if ($cmsmasters_project_title == 'true') {
		echo '<header class="cmsmasters_project_header entry-header">';
			psychology_help_project_title_nolink(get_the_ID(), 'h1');
		echo '</header>';
	}
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if (!post_password_required()) {
			if (sizeof($cmsmasters_project_images) > 0 && $cmsmasters_project_images[0] != '') {
					...
					echo '<div class="' . $cmsmasters_project_img_columns . '">' . 
						'<figure class="cmsmasters_img_rollover_wrap preloader">' . 
							...
							'<div class="cmsmasters_img_rollover">' . 
								'<a href="' . esc_url($link_href[0]) . '" rel="ilightbox[img_' . get_the_ID() . ']" title="' . (($image_atts['title'] != '') ? esc_attr($image_atts['title']) : cmsmasters_title(get_the_ID(), false)) . '" class="cmsmasters_image_link">' . esc_html__( 'Zoom', 'psychology-help' ) . '</a>' . 
							'</div>' . 
						'</figure>' . 
					'</div>';
				...
				psychology_help_thumb(get_the_ID(), 'cmsmasters-full-masonry-thumb', false, 'img_' . get_the_ID(), true, true, false, true, false);
			}
		}
		...
		echo '<div class="project_sidebar">';
			...
			if ($project_details == 'true') {
				echo '<div class="project_details entry-meta">' . 
					...
					psychology_help_get_project_likes('post');
					
					psychology_help_get_project_comments('post');
					
					psychology_help_get_project_author('post');
					
					psychology_help_get_project_date('post');
					
					psychology_help_get_project_category(get_the_ID(), 'pj-categs', 'post');
					
					psychology_help_get_project_tags(get_the_ID(), 'pj-tags');
					
					psychology_help_get_project_features('details', $cmsmasters_project_features, false, 'h3', true);
					
					psychology_help_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
					
				echo '</div>';
			}
			
			
			psychology_help_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h3', true);
			
			psychology_help_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h3', true);
			
			psychology_help_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h3', true);
