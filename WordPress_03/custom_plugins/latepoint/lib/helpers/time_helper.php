...
class OsTimeHelper {
  ...
  public static function format_date_with_locale($format, $date_obj){
    return date_i18n($format, $date_obj->format('U'));
  }
  ...
  public static function get_nice_date_no_year($date){
    ...
    if($d->format('Y') == OsTimeHelper::today_date('Y')){
      return date_i18n("F j", $d->format("U"));
    }else{
      return date_i18n("M j, Y", $d->format("U"));
    }
  }
    ...
    if(self::is_timezone_saved_in_session()){
      ...
      if(OsAuthHelper::is_customer_logged_in()){
        ...
        if(!empty($customer_timezone_name)){
          self::set_timezone_name_in_session($customer_timezone_name);
        ...
        self::set_timezone_name_in_session(self::get_wp_timezone_name());
        $timezone_name = self::get_wp_timezone_name();
      }
    }
